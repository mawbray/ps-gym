...
class SingleStageParallelLMaster(gym.Env):
    '''
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.due_dates      = [10, 22, 25, 20, 28, 30, 17, 23, 30, 21, 30, 28, 15, 29, 12]                  # defining due date of each of the 10 orders/products to be made
        ...
        self.min_var_p  = 0                                                        # lower bound of uniform processing time variation (periods from the mean)
    ...
    def _update_state(self):
        ...
        if t == 0: 
            state[m+u:m+2*u] = np.array(list(self.time_2_comp.values())).flatten()
        if t > 0:
            state[m+u:m+2*u] = np.array(list(self.time_2_comp.values())).flatten()
        # due date 
        if t == 0: 
